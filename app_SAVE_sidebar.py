...
# Specify the file path of the GeoJSON file
file_path = "landkreise.geojson"

# Read the GeoJSON file using geopandas
landkreise = gpd.read_file(file_path)

geometry = gpd.read_file("geometry.geojson")
...
def main():
    ...
    selected_features = st.sidebar.multiselect('Select 3 parameters from the list, that you like most', all_features#default=[] or all_features[:1]
                                       , max_selections=4)
    ...
    # Use JavaScript code to hide the multi-select dropdown list after selecting options
    if selected_features:
        st.markdown("<style>.css-1g6dbo0 { display: none; }</style>", unsafe_allow_html=True)
