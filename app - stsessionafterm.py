    ...
    for feature in all_features:
        if st.sidebar.button(feature, key=feature):
            ...
            if feature not in selected_features:
                st.session_state['parameter'] = selected_features.append(feature)
                ...
                for idx, feature in enumerate(all_features):
                    key = f"{feature}_{idx}"  # Generate a unique key for each button
                    if st.sidebar.button(feature, key=key):
                        if feature not in st.session_state['parameter']:
                            st.session_state['parameter'].append(feature)
                            selected_features_placeholder.text(f"Selected Features2: {', '.join(st.session_state['selected_features'])}")
                            time.sleep(60)  # Wait for 1 second after each selection
                        else:
    ...
    if len(selected_features) < 3:
        ...
        try:    
            ...
            if st.sidebar.button("Reset"):
                ...
                selected_features_placeholder.empty()
