...
def main():
    ...
    submit_button = None
    st.session_state['parameter'] = selected_features
    ...
    if submit_button is not None and submit_button:
        ...
        if selected_features:
            st.session_state['parameter'].extend(selected_features)
            selected_features_placeholder.text(f"Selected Features2: {', '.join(st.session_state['parameter'])}")
            time.sleep(1)  # Wait for 1 second after each selection
        ...
        try:    
            ...
            with st.form("my_form"):
                ...
                if submitted is not None and submit_button:
                    if len(selected_features) < 3:
                        st.warning('Please select at least three parameters.')
